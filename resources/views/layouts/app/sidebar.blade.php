    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:sidebar sticky collapsible="mobile" class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            @if ($organizations->count() > 0)
                <div class="px-4 py-2">
                    <flux:text class="mb-2 text-xs font-medium tracking-wide text-zinc-500 uppercase">{{ __('Organization') }}</flux:text>
                    <flux:dropdown class="w-full">
                        <flux:button variant="ghost" class="w-full justify-between" icon-trailing="chevron-down">
                            {{ $currentOrganization?->name ?? __('Select organization') }}
                        </flux:button>
                        <flux:menu>
